...
export default function AuthModal({ mode = 'login', onClose, onSwitchMode }: { mode?: AuthMode, onClose: () => void, onSwitchMode?: (m: AuthMode) => void }) {
  ...
  const node = (
    <div onClick={onClose} className="fixed inset-0 z-[9999] flex items-center justify-center p-4 min-h-screen" style={{ background: 'rgba(10,11,13,0.35)', backdropFilter: 'blur(6px)' }} role="dialog" aria-modal="true">
      <div onClick={(e) => e.stopPropagation()} className="relative w-full max-w-[520px] mx-auto bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[90vh] overflow-auto" style={{ overflow: 'hidden' }}>
        <div className="flex items-center justify-between px-6 pt-6 pb-2 bg-white border-b border-gray-100">
          <h3 className="text-xl font-bold text-gray-800">{mode === 'login' ? 'Đăng nhập' : 'Đăng ký'}</h3>
          <button aria-label="Đóng" className="w-9 h-9 flex items-center justify-center rounded-full bg-[#f1f3f6] hover:bg-[#e6e9ee] transition" onClick={onClose}>
        ...
        <div className="px-6 pb-8 pt-6 bg-white">
          <div className="space-y-4">
            ...
            <div className="pt-1">
              <Button className="w-full bg-gradient-to-r from-[#36d1c4] via-[#5b86e5] to-[#f857a6] text-white">{mode === 'login' ? 'Đăng nhập' : 'Đăng ký'}</Button>
            </div>

            <div className="text-center text-sm text-gray-500">
              {mode === 'login' ? (
                <>Chưa có tài khoản? <button className="text-blue-500 font-medium" onClick={() => onSwitchMode ? onSwitchMode('register') : null}>{'Đăng ký ngay'}</button></>
              ) : (
                <>Đã có tài khoản? <button className="text-blue-500 font-medium" onClick={() => onSwitchMode ? onSwitchMode('login') : null}>{'Đăng nhập'}</button></>
              )}
            </div>
          </div>
        </div>
